...
import com.example.model.Header;
...
public class TokenHandler extends FunctionInvoker<TokenDetail, Greeting> {
	...
	public HttpResponseMessage execute(
		...
		TokenDetail tokendetail = request.getBody().filter((u -> u.getToken() != null))
				.orElseGet(() -> new TokenDetail(request.getQueryParameters().getOrDefault("token", "empty")));
		...
		tokendetail.setHeader(mapper.readValue(headerJson, Header.class));
		tokendetail.setPayload(mapper.readValue(payloadJson, Payload.class));
		return request.createResponseBuilder(HttpStatus.OK).body(handleRequest(tokendetail, context))
